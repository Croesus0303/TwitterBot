    ...
    def send_tweet(tweet):
        ...
        if tweet.imageUrl is not None:
            try:
                ...
                logging.info("Download image is succesful" + tweet.imageUrl)
            except Exception as e :
                logging.warning("Download image is unsuccesful" + str(e))

            try:
                media_id = get_media_id(tweet, api_key, api_key_secret,
                                    access_token, access_token_secret, file_name)
                logging.info("Upload image to twitter is succesful")
            except Exception as e :
                logging.warning("Upload image is unsuccesful" + str(e))
        ...
        if media_id is not None:
            try:
                ...
                os.remove(file_name)
                logging.info("Create tweet api is success")
            except Exception as e:
                logging.warning("Create tweet api is unsuccess" + str(e))
        else:
